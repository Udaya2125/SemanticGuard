...
class GeminiAgent(AgentProvider):
    """
    An agent provider that uses Google's Gemini model.
    """

    def __init__(self):
        try:
            self.client = genai.Client(api_key=settings.GEMINI_API_KEY)
            self.model_name = settings.GEMINI_AGENT_MODEL
            logger.info("Gemini Agent initialized successfully.")
        except Exception as e:
            logger.error(f"Failed to initialize Gemini Agent: {e}", exc_info=True)
            raise

    def generate_response(self, mode: str, query: str, context: Optional[Dict[str, Any]] = None) -> str:
        """
        Generates a response using the Gemini model based on the specified mode.
        """
        prompt = self._get_prompt_for_mode(mode, query, context)
        
        try:
            logger.info(f"Generating Gemini response for mode '{mode}' with query '{query}'")
