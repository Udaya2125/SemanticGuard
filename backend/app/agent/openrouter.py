...
class OpenRouterAgent(AgentProvider):
    """
    An agent provider that uses an OpenRouter-hosted model via the
    OpenAI-compatible chat completions API. Same interface and same
    per-mode prompt behavior as the previous GeminiAgent implementation
    (see app/agent/gemini.py) — only the LLM provider changed.
    """
    ...
    def __init__(self):
        try:
            ...
            logger.error(f"Failed to initialize OpenRouter Agent: {e}", exc_info=True)
            raise

    def generate_response(self, mode: str, query: str, context: Optional[Dict[str, Any]] = None) -> str:
        """
        Generates a response using the configured OpenRouter model based on
        the specified mode.
        """
        prompt = self._get_prompt_for_mode(mode, query, context)

        try:
            logger.info(f"Generating OpenRouter response for mode '{mode}' with query '{query}'")
    ...
    def _get_prompt_for_mode(self, mode: str, query: str, context: Optional[Dict[str, Any]] = None) -> str:
        ...
        Identical prompt text to the previous Gemini implementation.
        """
        import json
...
# Singleton instance
agent_provider = OpenRouterAgent()
