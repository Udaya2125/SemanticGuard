...
class GrokFactJudge(FactJudgeProvider):
    """
    A Fact Judge provider that uses an xAI Grok model via the
    ...
    OpenRouterFactJudge (see app/judge/openrouter.py) — only the LLM
    ...
    def __init__(self):
        try:
            ...
            logger.info("Grok Fact Judge initialized successfully.")
        except Exception as e:
            logger.error(f"Failed to initialize Grok Fact Judge: {e}", exc_info=True)
            raise
    ...
    def judge_leak(self, agent_output: str, protected_doc: VaultDocument) -> FactJudgeDetails:
        ...
        for attempt in range(3):  # Retry up to 3 times
            try:
                logger.info("Requesting factual overlap judgment from Grok...")
                ...
                logger.error(f"Attempt {attempt + 1}: Failed to decode JSON from Grok response: {e}. Response text: {raw_text}")
                if attempt == 2:
    ...
    def _parse_json_response(self, raw_text: str) -> Dict[str, Any]:
        """
        Parses the model's JSON output robustly. If the selected Grok model
        ...
        contract used by OpenRouterFactJudge.
        """
        text = raw_text.strip()
    ...
    def _create_prompt(self, agent_output: str, protected_doc: VaultDocument) -> str:
        """Creates the detailed prompt for the model. Identical to the OpenRouter/Gemini prompt."""
...
# Singleton instance
fact_judge_provider = GrokFactJudge()
