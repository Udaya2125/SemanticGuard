...
class OpenRouterFactJudge(FactJudgeProvider):
    """
    A Fact Judge provider that uses an OpenRouter-hosted model via the
    OpenAI-compatible chat completions API to detect factual overlap.
    Same interface, same prompt, same retry/validation contract as the
    previous GeminiFactJudge implementation (see app/judge/gemini.py) —
    only the LLM provider changed.
    """

    def __init__(self):
        try:
            self.client = OpenAI(base_url=OPENROUTER_BASE_URL, api_key=settings.OPENROUTER_API_KEY)
            self.model_name = settings.OPENROUTER_MODEL
            logger.info("OpenRouter Fact Judge initialized successfully.")
        except Exception as e:
            logger.error(f"Failed to initialize OpenRouter Fact Judge: {e}", exc_info=True)
            raise
    ...
    def judge_leak(self, agent_output: str, protected_doc: VaultDocument) -> FactJudgeDetails:
        ...
        for attempt in range(3):  # Retry up to 3 times
            try:
                logger.info("Requesting factual overlap judgment from OpenRouter...")
                ...
                logger.error(f"Attempt {attempt + 1}: Failed to decode JSON from OpenRouter response: {e}. Response text: {raw_text}")
                if attempt == 2:
    ...
    def _parse_json_response(self, raw_text: str) -> Dict[str, Any]:
        """
        Parses the model's JSON output robustly. The requested model supports
        OpenRouter's `response_format: json_object`, but some providers still
        wrap the object in markdown fences or leading/trailing prose, so this
        falls back to extracting the first JSON object in the text before
        giving up — the same "safest reliable parsing" contract requested for
        models without guaranteed structured output.
        """
        text = raw_text.strip()
    ...
    def _create_prompt(self, agent_output: str, protected_doc: VaultDocument) -> str:
        """Creates the detailed prompt for the model. Identical to the previous Gemini prompt."""
...
# Singleton instance
fact_judge_provider = OpenRouterFactJudge()
